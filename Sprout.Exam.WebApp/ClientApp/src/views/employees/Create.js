...
export class EmployeeCreate extends Component {
  ...
  constructor(props) {
    super(props);
      this.state = { fullName: '', birthdate: '', tin: '', typeId: 1, loading: false, loadingSave: false, employeesType:[] };
  }

    componentDidMount() {
        this.populateEmployeeTypeInfo();
  }
...
<div className='form-group col-md-6'>
  ...
  <select id='inputEmployeeType4' onChange={this.handleChange.bind(this)} value={this.state.typeId}  name="typeId" className='form-control'>
                            {this.state.employeesType.map((e, i) => {
                                return (<option key={i} value={e.id}>{e.TypeName}</option>)
                            })}
  </select>
</div>
</div>
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
    ...
    async populateEmployeeTypeInfo() {
        const token = await authService.getAccessToken();
        const axios = require('axios').default;
        axios.get("api/employees/type", { headers: !token ? {} : { 'Authorization': `Bearer ${token}` } }).then(res => {
            this.setState({ employeesType: res.data });
        })
    }
